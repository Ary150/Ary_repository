...
export function modLevelAndXP(){
  ...
  console.log('\x1b[33m\x1b[45m^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\x1b[0m\x1b[0m');
    ...
    while ((playerRandom1.hp >= 0 || enemyRandom1.hp >= 0) && i < y) {
        ...
        console.log('Tu as : '+levelPlayer+' de level :D');
        console.log('Tu as : '+xpPlayer+' d\' XP ;)');
      ...
      if (enemyRandom1.hp <= 0) {
        ...
        if(xpPlayer > 10 || xpPlayer > 20 || xpPlayer > 25 || xpPlayer > 30 || xpPlayer > 35){
          levelPlayer+=1;
          playerRandom1.hp = playerRandom1.hp*(3 / 2);
          playerRandom1.str = playerRandom1.str*(3 / 2)
        }
